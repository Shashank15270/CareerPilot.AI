...
export default function History() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 sm:py-12 relative min-h-[85vh] text-left overflow-hidden">
      {/* Glow highlight */}
        ...
        <div className="mt-8">
          {loading && (
            <div className="py-20 flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-sm font-semibold text-muted">Fetching history from database...</p>
            </div>
          )}
          ...
          {error && !loading && (
            <div className="max-w-md mx-auto p-6 rounded-2xl bg-red-500/10 border border-red-500/25 flex flex-col items-center space-y-3 text-red-400 text-center">
              ...
              <h4 className="font-bold text-red-700">Database Error</h4>
                    ...
                    {data.length === 0 ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {data.map((job) => (
                          <div
                            key={job.id}
                            className="p-5 rounded-2xl bg-white/60 border border-zinc-200 hover:border-zinc-300 transition-all flex flex-col justify-between"
                          >
                            <div className="space-y-2">
                              <div className="flex justify-between items-start gap-4">
                                <div>
                                  <h3 className="font-bold text-text text-base leading-snug">{job.title}</h3>
                            ...
                            <div className="flex justify-between items-center border-t border-zinc-200 pt-3 mt-4 text-[10px] text-muted uppercase font-bold">
                    ...
                    {data.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        {data.map((resume) => (
                          <div
                            key={resume.id}
                            className="p-4 rounded-xl bg-white/60 border border-zinc-200 flex items-center justify-between gap-4 hover:bg-surface transition-all"
                          >
                            <div className="flex items-center space-x-3.5">
                              ...
                              <div>
                                <h4 className="font-semibold text-text text-sm truncate min-w-0 flex-1">
                            ...
                            <div className="text-[10px] text-muted font-bold bg-surface border border-zinc-300 px-2.5 py-1 rounded-md uppercase font-mono">
                              DB ID: {resume.id}
                            </div>
                          </div>
                        ))}
                    ...
                    {data.length === 0 ? (
                      ...
                      <div className="space-y-4">
                        {data.map((rec) => (
                          <div
                            key={rec.id}
                            className="p-5 rounded-2xl bg-white/60 border border-zinc-200 space-y-3 hover:border-zinc-300 transition-all"
                          >
                            <div className="flex justify-between items-start text-xs">
                              <div>
                                <span className="text-[10px] text-muted uppercase font-bold tracking-wider">Search Keyword / Query:</span>
                                <h4 className="font-bold text-text text-sm mt-0.5">
                            ...
                            <div className="border-t border-zinc-200 pt-3">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {rec.results_json?.slice(0, 3).map((job, idx) => (
                                  <div
                                    key={idx}
                                    className="px-2.5 py-1.5 rounded-lg bg-surface border border-zinc-200 text-[11px] text-text"
                                  >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-1 space-y-3 max-h-[60vh] overflow-y-auto pr-2">
                          ...
                          {data.map((session) => (
                            ...
                            >
                              <h4 className="font-bold text-text text-sm">{session.job_title}</h4>
                              <p className="text-xs text-muted/70 font-semibold">{session.job_company}</p>
                              <div className="flex justify-between items-center text-[10px] text-muted mt-3 font-semibold pt-1 border-t border-zinc-200">
                        ...
                        <div className="lg:col-span-2 md:col-span-2 bg-white/60 border border-zinc-200 rounded-3xl p-6 min-h-[40vh] flex flex-col">
                          {!selectedInterview ? (
                            ...
                            <div className="space-y-6 flex-grow flex flex-col">
                              ...
                              <div className="border-b border-zinc-200 pb-4">
                                <h3 className="text-lg font-bold text-text">Scorecard: {selectedInterview.job_title}</h3>
                              ...
                              {loadingDetails ? (
                                ...
                                <div className="space-y-4 flex-grow overflow-y-auto max-h-[50vh] pr-2">
                                  {interviewHistoryDetails.map((historyItem, idx) => (
                                    <div
                                      key={historyItem.id}
                                      className="p-4 bg-surface border border-zinc-200 rounded-xl space-y-2.5"
                                    >
                                      <div className="flex justify-between items-start gap-4 text-xs font-bold">
                                        <span className="text-text">Q{idx + 1}: {historyItem.question}</span>
                                      ...
                                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 text-center pt-1">
                                        {[
                                          ...
                                          <div key={i} className="p-1.5 bg-white/60 border border-zinc-200 rounded-lg">
                                            <span className="text-[8px] text-muted uppercase font-bold block">{dim.name}</span>
                                            <span className="text-xs font-bold text-text">{dim.val || 'N/A'}</span>
                                          </div>
                                        ))}
                                      </div>
                                    </div>
                    ...
                    {data.length === 0 ? (
                      ...
                      <div className="space-y-4">
                        {data.map((report) => (
                          <div
                            key={report.id}
                            className="p-5 rounded-2xl bg-white/60 border border-zinc-200 space-y-4 hover:border-zinc-300 transition-all text-left"
                          >
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                              <div className="p-4 rounded-xl bg-white/60 border border-zinc-200 space-y-2">
                                ...
                                <div className="space-y-1">
                                  {report.report_json?.career_path?.map((step, i) => (
                                    <div key={i} className="flex items-center space-x-2 text-xs text-text">
                                      <span className="w-4 h-4 rounded-full bg-surface text-[10px] font-bold flex items-center justify-center">{i+1}</span>
                                      <span>{step}</span>
                                    </div>
                                  ))}
                                </div>
...
function EmptyState({ message }) {
  return (
    <div className="py-16 text-center text-muted max-w-sm mx-auto space-y-4">
      <div className="w-12 h-12 rounded-full bg-white/60 border border-zinc-200 flex items-center justify-center mx-auto">
